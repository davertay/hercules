...
# generated, not one the test invented. The payload is the real StopFailure shape, whose failure class
# rides on `error`. Then we fail the way the harness fails on an API error: an is_error result on
# stdout carrying its wording, and a non-zero exit with empty stderr.
settings=""
prev=""
for arg in "$@"; do
...
payload='{"session_id":"00000000-0000-0000-0000-000000000000","hook_event_name":"StopFailure",'
payload+='"error":"rate_limit","last_assistant_message":"You'"'"'ve hit your session limit"}'
echo "$payload" | eval "$hook"
